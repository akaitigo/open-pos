...
export interface UseCheckoutReturn {
  ...
  handleMethodChange: (nextMethod: string) => void
  handleCashKeypadPress: (key: string) => void
  handleAddCurrentPayment: () => void
  handleRemovePayment: (paymentId: string) => void
...
export function useCheckout(
  ...
  function handleMethodChange(nextMethod: string) {
    if (!isPaymentMethod(nextMethod)) return
    setPaymentMethod(nextMethod)
    if (nextMethod !== 'CASH' && !paymentAmount) {
