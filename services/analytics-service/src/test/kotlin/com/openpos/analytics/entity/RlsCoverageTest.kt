 ...
 * analytics-service の全エンティティが organizationFilter を持つことを検証する。
 *
 * 対象外:
...
class RlsCoverageTest {
    /**
     * organizationId を持つ全エンティティクラスに @FilterDef と @Filter が定義されていることを検証する。
     */
    ...
    @ValueSource(
        classes = [
            ...
            ProductSalesEntity::class,
            SalesTargetEntity::class,
            ProductAlertEntity::class,
        ],
    )
    ...
    fun `organizationId フィールドが全テナントエンティティに存在する`() {
        val tenantEntities =
            listOf(
                DailySalesEntity::class.java,
                HourlySalesEntity::class.java,
                ProductSalesEntity::class.java,
