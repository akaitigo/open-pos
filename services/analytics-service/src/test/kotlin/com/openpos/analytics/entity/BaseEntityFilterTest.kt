 ...
 */
@Entity
@Table(name = "test_analytics_entity")
class TestAnalyticsEntity : BaseEntity() {
    var name: String = ""
}
...
class BaseEntityFilterTest {
    ...
    fun `BaseEntity has organizationId field`() {
        val entity = TestAnalyticsEntity()
        entity.organizationId = UUID.randomUUID()
        entity.name = "test-analytics"
    ...
    fun `organizationFilter filters entities by organizationId`() {
        ...
        val results =
            entityManager
                .createQuery("SELECT e FROM TestAnalyticsEntity e", TestAnalyticsEntity::class.java)
                .resultList
